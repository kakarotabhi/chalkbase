 ...
 *
 * <p>{@code academics} is a container: it has no screen of its own, and it declares all of its
 ...
 * choice {@code SchoolNavigation} makes for the profile screen, and deliberately: these three
 * screens are worth opening to look at — a subject teacher checking which sections Class 9 has, or
 * which subjects the school teaches — where the profile screen exists only to edit.
 */
@Configuration
    ...
    NavigationProvider academicsNavigationProvider() {
        return () -> List.of(new NavigationItem(
                ...
                List.of(
                        ...
                        new NavigationItem(
                                ACADEMICS_SUBJECTS,
                                "nav.academics.subjects",
                                "book",
                                30,
                                AcademicsPermissions.SUBJECT_READ))));
    }
}
