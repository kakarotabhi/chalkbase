...
| Where do login accounts live? | Inside each school's schema, so the school code is part of signing in. Sessions are the single exception in `public`, because reading the cookie is what tells us which tenant to bind. Parents sign in with a school-issued username; a guardian is a person record that gets an account only when one is needed. | [ADR-0017](../architecture/adr/0017-identity-model.md) |
| How do we version the API? | We do not. One client ships with the backend, so a breaking change is one atomic pull request. Paths carry no version segment at all. Revisit when a consumer exists that cannot be forced to upgrade. | [ADR-0016](../architecture/adr/0016-no-api-versioning.md) |
| How is one installation split between schools? | One PostgreSQL schema per school in a single database, one connection pool. A multi-campus group is one schema **per campus**, with the group as a row in `public`. Migrations fan out to every schema at startup; a tenant registry in `public` lists them. | [ADR-0011](../architecture/adr/0011-schema-per-tenant.md) |
