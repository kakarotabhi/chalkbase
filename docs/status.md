...
fills in for no reason. Documents used to be the second, until the five `CHALKBASE_STORAGE_*`
variables were set on Render — they are set now, and the round trip is verified on the deployed
environment ([document storage](operations/document-storage.md)). A deployment without them still
answers a clean 503 rather than failing oddly, which is what makes that an operational step rather
than a gap. Transport and hostel are recorded under _Decisions taken and not to be reopened_ rather
...
| Circulars and notices | `communication` | ✅ Done — compose, target by class/section, publish and per-recipient acknowledgement ([ADR-0035](architecture/adr/0035-circular-recipients-and-no-channel-port.md)); SMS/WhatsApp/email dispatch and multilingual templates still not started |
| Leave requests | `attendance` | ✅ Done — filed against a section's live roster, approved leave read at mark time and defaulted to `EXCUSED_LEAVE` rather than written ahead of the day ([ADR-0030 amendment](architecture/adr/0030-attendance-grain-and-lock.md)) |
...
| **Phase 2:** leave requests (FR-047) — filed by staff on a parent's behalf against a section's live roster, refused for a start date already in the past (that is what the correction-request flow is for), approved or rejected by the same three role templates that manage the section's attendance. Approving never writes `attendance_mark` directly: the marking screen reads an approved request when a section's register is opened for the covered date and offers `EXCUSED_LEAVE` as an unmarked student's default, which a teacher may still mark otherwise | [ADR-0030 amendment](architecture/adr/0030-attendance-grain-and-lock.md), `attendance/`, `features/attendance/leave-request-list.ts` |
| **Phase 2:** circulars and notices end to end — compose one, target it by class or whole class (a live recipient-count preview while composing), publish it, and see or record per-recipient delivery and acknowledgement. A recipient is a student id, not a guardian account (there is no parent login yet); no `NotificationChannel` adapter is registered, because an in-app circular is a row this system already holds, not something delivered through an external channel | [ADR-0035](architecture/adr/0035-circular-recipients-and-no-channel-port.md), `communication/`, `features/communication/` |
...
| **Phase 2, fee structure:** fee heads and concession types (catalogues, not per session) and a session-scoped, versioned fee structure per class — amount, `FeeHeadCategory`, `InstallmentFrequency` and due dates per fee head. Never edited in place: `PUT /api/fees/structures/{session}/{class}` always writes a new version and supersedes the previous one in the same transaction, and a session that has already run its course refuses a second version (`FEE_009`) — only its very first version may ever be backfilled after the fact. Delhi's Development Fee cap is enforced as a percentage of tuition, set on the fee head rather than hardcoded. "Copy from a previous session" is an explicit, reported action that never overwrites a class already set up in the destination session, with due dates shifted by the gap between the two sessions' start dates. **Not built in this lane:** fee demand, invoices/charges, payments, receipts, dues, and applying a concession type to any student — see [ADR-0033](architecture/adr/0033-fee-structure-versioning.md) and the module's own package-info for exactly what FR-076's other targeting axes (section, student category, transport route, hostel, individual student) are deferred to and why. | [ADR-0012](architecture/adr/0012-fee-ledger-model.md), [ADR-0033](architecture/adr/0033-fee-structure-versioning.md), `fee/`, `features/fees/` |
| Three findings from driving the deployed app fixed: the fee structure editor's Save button now says what it is waiting for (a fee head, an amount, or the previously-unstated due date) instead of sitting greyed out with no reason given; the leave-request list and detail screens render their date ranges and decision timestamp the way the rest of the product does (`9 Sept 2026`, and the decision time in the school's own zone, ADR-0032) instead of raw `yyyy-MM-dd` and a raw ISO instant, using a new shared `instantFormat`/`formatInstant` pair in `shared/formatting/day.ts` that the audit log now also uses in place of its own local copy; and the locked-attendance-date banner no longer points at a per-row "Request a correction" control on a date where nothing was ever marked and no such control renders anywhere on the page. | `features/fees/fee-structure.ts`, `features/attendance/leave-request-detail.ts`, `features/attendance/leave-request-list.ts`, `features/attendance/attendance-mark.html`, `features/audit/audit-log.ts`, `shared/formatting/day.ts` |
...
| Fixed on every profile: Boot auto-configured an in-memory `UserDetailsManager` and logged a fresh random password for it on every boot, even though nothing in this codebase ever asks a Spring Security `AuthenticationManager` for a principal — `AuthenticationService` verifies a password against `user_credential` and builds the `SecurityContext` itself, and `SecurityConfig` never calls `httpBasic()`/`formLogin()`, so the generated user was unreachable by any request; `UserDetailsServiceAutoConfiguration` is now excluded, pinned by a `prod`-profile test | `application.yml`, `UserDetailsServiceAutoConfigurationTests` |

| Fixed: a form could be submitted invalid without saying why — `cb-form-field` has always rendered `[error]` with `role="alert"` and handed back `describedById()`/`hasError()`, but 73 of 132 callers never passed it, so pressing Save on an invalid field (an email typed as `not-an-email`, a name left blank) did nothing visible and nothing audible. `[error]` and `[invalid]` are now wired up on `student-medical`, `student-contact`, `student-previous-school`, `fee-heads` and `enquiry-detail` (the reassign and follow-up forms), and on `circular-list`'s title and body; each message names the actual failing validator (required, max length, email shape, or — on the follow-up form's next-date field — the same next-follow-up-required cross-field rule `StudentCompliance`'s APAAR consent check already used the pattern for). `student-import`'s two fields and several select/category controls elsewhere carry no validator that can actually fail and were left alone rather than gaining an invented one; the remaining 59 already-bound callers and any other unbound caller outside this list are unreviewed and open for a follow-up. | `shared/components/form-field/`, `features/students/`, `features/fees/fee-heads.ts`, `features/admissions/enquiry-detail.ts`, `features/communication/circular-list.ts` |

| Fixed, found driving the deployed app: once any account was locked out (AUTH_003), the sign-in form's Sign in button stayed `disabled` forever and the stale "Account locked" banner kept naming the wrong account, even after a different username was typed — recoverable only by reloading the page, which matters on a school office's one shared computer. `isLocked`/the banner now clear the moment any field's *value* actually changes (touching or tabbing through a field alone does not, deliberately — the banner names a fact about what was submitted, not about cursor position), so a different person can sign in immediately; resubmitting the same still-locked account is allowed and honestly returns AUTH_003 again rather than being blocked client-side with no way to tell it had unlocked. `change-password.ts` has the same staleness shape for `AUTH_006`/`AUTH_007`, left alone on purpose: its Save button isn't bound to its failure code at all, so it is a stale message, not a stuck form. | `features/auth/login/login.ts`, `features/auth/login/login.spec.ts` |
...
| Fixed: `location.search` stayed empty on every list screen's filters and page, so a filtered view could not be linked, bookmarked, or recovered with the back button. Status/section/source/counsellor/decision/action/date-range/actor-id filters and the page are now mirrored into the URL (replacing the current history entry, never pushing one) on the students, guardians, leave requests, enquiries, circulars and audit log screens; free-text search stays out of it everywhere, per ADR-0014. The account roster carries neither filters nor pagination today, so it needed no change. | `shared/routing/list-query-params.ts`, `features/students/`, `features/attendance/leave-request-list.ts`, `features/admissions/enquiry-list.ts`, `features/communication/circular-list.ts`, `features/audit/audit-log.ts` |
...
- The per-component style budget was raised from 4 kB / 8 kB to **8 kB / 12 kB**. The old warning
  ...
  already drifted.
- ~~`GET /api/access/users` answers `UserSummary` — id, display name, status — for every account
  in one call, and nothing else: no `lockedUntil`, no `lastLoginAt`. Only the four per-account
  write endpoints answer the richer `UserAccountResponse`. So the account roster
  (`/settings/users`) cannot paint a lockout badge per row without an N+1 request per account,
  which every list screen in this app avoids; **Clear lockout** is offered on every active account
  instead, and the confirmation that follows is where the truth about whether anything was locked
  actually comes from. Closing it properly means a paged, filterable roster endpoint carrying the
  fuller shape, which is a bigger change than the screen that surfaced the gap.~~ ✅ Closed,
  without that bigger endpoint: `UserSummary` gained one field, `locked` — the backend's own
  `lockedUntil`-versus-`Instant.now()` comparison (`AccessDirectory`), never the raw timestamp, so
  a lockout that has expired is not shown as one — rather than the richer `UserAccountResponse`
  shape this entry once called for. The roster needed a fact per row, not a login history; a
  boolean fits on the one-call list shape it already had. `/settings/users` now shows a **Locked**
  badge and offers **Clear lockout** only on a row that badge marks locked.
...
- **Attendance has no section-scoped permission narrowing.** `attendance:mark:manage` reaches
  ...
  introduced.
- **Leave requests (FR-047) have the same section-scoping gap, and one more of their own.**
  `attendance:leave:approve` reaches every section, not only a class teacher's, for the identical
  reason `attendance:mark:manage` does above. Separately, this lane does not prevent two requests
  for the same student from covering overlapping dates — a second request is accepted whether or
  not an earlier pending or approved one already covers the same days — and does not run anything
  to write a mark on a school's behalf if a section's register is never opened before an approved
  leave's date locks; that day simply stays unmarked, the same as any day nobody marked at all. Both
  are scoped decisions, not oversights — see the ADR-0030 amendment ("Leave requests: how an
  approval reaches the register") for the reasoning.
