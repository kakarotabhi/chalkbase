...
- **Documents has no screen and no working adapter on the deployed environment.**
  ADR-0025 built the port, the module and the endpoints, but `prod` registers a storage
  adapter that answers a clear 503 on every call until an S3-compatible one is chosen and
  configured — a dependency decision (AWS SDK v2 vs a hand-rolled SigV4 signer) left to the
  product owner. A document uploaded on `local` or `test` works end to end today; nothing
  uploaded on the deployed environment persists yet, and there is nowhere in the UI to try.
