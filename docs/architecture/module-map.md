...
| `attendance` | `attendance_mark`, `attendance_correction_request`, `attendance_leave_request` (per tenant) — student attendance, daily and period-wise in one table from day one; staff attendance still needs a `staff` module | `/api/attendance` | yes | daily grain built: mark, view, lock, correction request and approval (ADR-0030); leave requests (FR-047), approved leave read at mark time rather than written to the mark (ADR-0030 amendment); period-wise has its table shape and no write path |
