...
There was one, briefly: a second API and web pair on the `staging` branch with a Supabase project of
its own, added so a branch could be verified running before it reached `main`. It has been retired
at the product owner's request, and the reasoning is worth keeping rather than leaving a reader to
wonder why the branch and the second Supabase project still exist.

What it bought was real — it is where the deployment bootstrap
([ADR-0024](../architecture/adr/0024-bootstrap-deployment.md)) was proved to work against a genuinely
empty database, which is the one thing the dev environment could never test, because its school was
created by a developer's seeder rather than by the product. What it cost was a second environment to
keep current on every merge, on a free tier where a cold deploy times out at fifteen minutes.

With Phase 1 complete and `main` deploying to dev on every merge, the second pair was paying that
cost for a verification nobody was performing. **Verify on dev, after merge.** If a change is risky
enough to want a rehearsal, the honest options are a local run or bringing the pair back
deliberately — `git log` has the Blueprint that defined it.
